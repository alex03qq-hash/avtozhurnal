...
const SPECS: Record<CollectionName, Record<string, FieldSpec>> = {
  vehicles: {
    ...
    usageClass: { type: 'enum', label: 'Условия эксплуатации', values: ['highway', 'normal', 'city', 'severe', 'taxi'], default: 'normal' },
  ...
  rules: {
    ...
    notes: { type: 'text', label: 'Заметки', default: '' },
    code: { type: 'nullableId', label: 'Код пункта регламента' },
    origin: { type: 'enum', label: 'Источник пункта', values: ['user', 'pack'], default: 'user' },
    packId: { type: 'nullableId', label: 'Пакет регламента' },
    packTitle: { type: 'nullableId', label: 'Название пакета' },
    packRevision: { type: 'nullableNumber', label: 'Ревизия пакета', min: 0 },
    packNotes: { type: 'text', label: 'Заметки из пакета', default: null },
    manufacturerIntervalKm: { type: 'nullableNumber', label: 'Заводской интервал, км', min: 0 },
